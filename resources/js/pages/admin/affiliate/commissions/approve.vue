    ...
    <v-row align="center" justify="center">
      <v-col cols="12" md="6">
        <v-card>
          ...
          <v-card-text>
            <p>
              {{ $t('Are you sure you want to approve this commission?') }}
              {{ $t('User balance will be automatically increased by the commission amount.') }}
            </p>
            <v-form @submit.prevent="reject">
              <v-btn type="submit" color="green" :disabled="form.busy" :loading="form.busy">{{ $t('Approve') }}</v-btn>
            </v-form>
          </v-card-text>
        </v-card>
      </v-col>
...
export default {
  ...
  methods: {
    async reject () {
      const { data } = await this.form.post(`/api/admin/affiliate/commissions/${this.id}/approve`)
