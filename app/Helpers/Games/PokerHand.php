...
class PokerHand
{
    ...
    public function __construct(Collection $pocketCards, Collection $communityCards)
    {
        ...
        if ($this->getCards()->count() > 7) {
            throw new \Exception(sprintf('You should not pass more than %d cards in total, %d passed.', 7, $this->getCards()->count()));
        }

        $this->hand = collect();
        $this->kickers = collect();
    ...
    public function getKickers(): Collection
    {
        return $this->kickers;
    }
    ...
    public function compare(PokerHand $opponentHand): int
    {
        ...
        // if ranks are equal
            ...
            if ($rank == self::HAND_ROYAL_FLUSH) {
                ...
                // compare two of a kind if three of a kind ranks are equal, there are no kickers in thid case
     ...
     * @throws \Exception
     */
    protected function compareCardCollections(Collection $collection, Collection $collection2): int
    {
        ...
        // loop through cards
        foreach ($collection as $i => $card) {
            $result = $this->compareCards($card, $collection2->get($i));
    ...
    public function getHighCard(): ?Collection
    {
        return collect([$this->getCardsSortedByRank()->first()]);
    }

    public function getOnePair(): ?Collection
    {
    ...
    public function getTwoPair(): ?Collection
    {
        $hands = $this->getCardsOfSameRank(2);
    ...
    public function getThreeOfAKind(): ?Collection
    {
        $threes = $this->getCardsOfSameRank(3);

        return $threes->isNotEmpty() ? $threes->last() : NULL;
    }

    public function getStraight($flush = FALSE): ?Collection
    {
        $sequences = collect();
        $lastSuit = NULL;
        $lastRank = 0;
        ...
        $sequences = $sequences
            ->filter(function (Collection $sequence) {
                return $sequence->count() >= 5;
            });

        return $sequences->isNotEmpty()
            ? $sequences
                ->sortBy(function (Collection $sequence) {
    ...
    public function getFlush(): ?Collection
    {
    ...
    public function getFullHouse(): ?Collection
    {
        $threes = $this->getCardsOfSameRank(3);
        $twos = $this->getCardsOfSameRank(2);

        return $threes->isNotEmpty() && $twos->isNotEmpty() ? $threes->last()->concat($twos->last()) : NULL;
    }

    public function getFourOfAKind(): ?Collection
    {
    ...
    public function getStraightFlush(): ?Collection
    {
        return $this->getStraight(TRUE);
    }

    public function getRoyalFlush(): ?Collection
    {
        $flush = $this->getFlush();

        return $flush && $flush->map->value->map->code->diff(['T','J','Q','K','A'])->isEmpty() ? $flush : NULL;
    }
     ...
     * @return Collection
     */
    public function getCardsSortedByRank($acesLow = FALSE): Collection
    {
    ...
    protected function make(): PokerHand
    {
        ...
        if ($hand->count() < 5) {
            $handCardCodes = $hand->map->code;

            $this->kickers = $this->getCardsSortedByRank()
                // remove cards that are present in hand
                ->filter(function ($card) use ($handCardCodes) {
                    return !$handCardCodes->contains($card->code);
                })
                ->take(5 - $hand->count())
                ->values();
     ...
     * @return Collection
     */
    protected function getCardsSortedByRankAcesLow(): Collection
    {
     ...
     * @return Collection
     */
    protected function getCardsSortedByRankAcesHigh(): Collection
    {
    ...
    protected function getCardsOfSameRank(int $count): Collection
    {
        ...
        return $values
            ...
            ->sortByDesc(function (Collection $cards) {
                return $cards->first()->value->rank;
            })
            ->values();
    }

    public static function getTitle(int $hand): string
    {
